...
class TestSearchChunking:
    ...
    def test_small_result_is_returned_whole(self, mcp_context):
        ...
        with patch('emby_mcp.server.get_items', return_value={'success': True, 'items': make_items(3)}):
    ...
    def test_large_result_returns_a_usable_first_chunk(self, mcp_context):
        ...
        with patch('emby_mcp.server.get_items', return_value={'success': True, 'items': make_items(5)}):
            result = json.loads(search_for_item(title_or_album="track"))
...
class TestMaxChunkSize:
    ...
    def test_unusable_value_falls_back_to_default(self, value):
        ...
        assert get_max_chunk_size({'max_chunk_size': value}) == emby_server.DEFAULT_MAX_CHUNK_SIZE
...
class TestUserList:
    ...
    def test_users_are_returned_as_json(self, mcp_context):
        ...
        with patch('emby_mcp.server.get_users', return_value={'success': True, 'users': users}):
            assert json.loads(retrieve_user_list()) == users

    def test_failure_is_reported(self, mcp_context):
        with patch('emby_mcp.server.get_users', return_value={'success': False, 'error': 'no route to host'}):
...
class TestLibraryList:
    ...
    def test_libraries_are_returned_and_cached(self, mcp_context):
        ...
        with patch('emby_mcp.server.get_library_list', return_value={'success': True, 'items': libraries}):
    ...
    def test_failure_clears_the_cache(self, mcp_context):
        ...
        with patch('emby_mcp.server.get_library_list', return_value={'success': False, 'error': 'timed out'}):
            result = retrieve_library_list()
...
class TestSelectLibrary:
    ...
    def test_selection_is_saved_to_the_context(self, mcp_context):
        ...
        with patch('emby_mcp.server.set_current_library', return_value={'success': True, 'library': library}):
    ...
    def test_library_list_is_fetched_when_not_already_cached(self, mcp_context):
        ...
        with patch('emby_mcp.server.get_library_list', return_value={'success': True, 'items': [library]}) as fetch, \
             patch('emby_mcp.server.set_current_library', return_value={'success': True, 'library': library}):
            assert select_library("Music") == 'Success'
    ...
    def test_unknown_library_name_is_reported(self, mcp_context):
        ...
        with patch('emby_mcp.server.set_current_library',
...
class TestGenreList:
    ...
    def test_genres_are_returned_for_the_current_library(self, selected_library):
        with patch('emby_mcp.server.get_genre_list',
    ...
    def test_failure_is_reported(self, selected_library):
        with patch('emby_mcp.server.get_genre_list', return_value={'success': False, 'error': 'bad request'}):
            result = retrieve_genre_list()
...
class TestSearchArguments:
    ...
    def test_supplied_arguments_are_passed_through(self, selected_library):
        with patch('emby_mcp.server.get_items', return_value={'success': True, 'items': []}) as get:
            search_for_item(
    ...
    def test_search_failure_is_reported_as_json(self, selected_library):
        ...
        with patch('emby_mcp.server.get_items', return_value={'success': False, 'error': 'query rejected'}):
            result = json.loads(search_for_item(title_or_album="anything"))
    ...
    def test_a_new_search_discards_the_previous_saved_results(self, selected_library):
        ...
        with patch('emby_mcp.server.get_items', return_value={'success': True, 'items': make_items(1)}):
...
class TestVersion:
    ...
    def test_reported_version_matches_the_package(self):
        ...
        assert emby_server.MY_VERSION == __version__
